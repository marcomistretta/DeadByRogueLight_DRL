...
class Runner:
    ...
    def run(self):
        ...
        while self.ep <= self.total_episode:
            ...
            if self.frequency_mode == 'episodes':
                ...
                # Initialize queue
                rollout = Queue(maxsize=len(self.envs))
                # Take the current weights of the network
                current_weights = self.agent.get_weights()

                # For each worker
                for w in self.envs:
                    # Update its local network
                    w.update_network_weight.remote(current_weights)
                    # Start the episode
                    w.mp_run.remote(rollout)

                # Wait until the rollout queue is full
                while not rollout.full():
                    continue

                # Get the workers parallel buffers and update the centralized one
                # TODO: can be improved?
                b_i = 0
                while not rollout.empty():
                    current_buffer = rollout.get()
                    for k in current_buffer.keys():
                        self.parallel_buffer[k][b_i] = current_buffer[k]
                    b_i += 1

                self.ep += len(self.envs)
            else:
                ...
                if self.recurrent:
                    threads = self.create_act_threds(agent=self.agent, parallel_buffer=self.parallel_buffer,
                                                     config=config,
                                                     states=states, num_steps=self.frequency, internals=internals,
    ...
    def compute_intrinsic_rewards(self, agent, with_extrinsic=True):
        if self.motivation is not None:
            num_batches = 10
            ...
            # If only last reward
            correction = []
            for l in agent.buffer['episode_lengths']:
                l_correction = np.zeros(l)
                l_correction[-1] = 1
                correction.extend(list(l_correction))
            intrinsic_rews *= np.asarray(correction)
