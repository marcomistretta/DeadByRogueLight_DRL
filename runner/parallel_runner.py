...
class EpisodeThreaded(Thread):
    def __init__(self, env, parallel_buffer, agent, index, config, num_episode=1, recurrent=False, motivation=False,
                 reward_model=False, motivation_model=None):
        self.env = env
        self.parallel_buffer = parallel_buffer
        self.agent = agent
        self.index = index
    ...
    def run(self) -> None:
        ...
        for i in range(self.num_episode):
            ...
            while not done:
                ...
                actions, logprobs, probs, dist = self.agent(torch.from_numpy(np.asarray([state])).to(device))
...
class Runner:
    def __init__(self, agent, frequency, envs, save_frequency=3000, logging=100, total_episode=1e10, curriculum=None,
                 ...
                 # IRL
                 reward_model=None, fixed_reward_model=False, dems_name='', reward_frequency=30, p_name_for_dems="",
                 dem_loader = None,
                 # Adversarial Play
    ...
    def create_episode_threads(self, parallel_buffer, agent, config):
        ...
        for i, e in enumerate(self.envs):
            ...
            threads.append(EpisodeThreaded(env=e, index=i, agent=agent, parallel_buffer=parallel_buffer, config=config,
                                           ...
                                           reward_model=(self.reward_model is not None), motivation_model=self.motivation))
        ...
        while self.ep <= self.total_episode:
            ...
            if self.frequency_mode == 'episodes' and self.ep > 0 and self.ep % self.logging == 0 or \
                ...
                if self.frequency_mode == 'episodes':
                    ...
                    print('Mean of {} episode reward after {} episodes: {}'.
                          format(self.logging, self.ep, np.mean(self.history['episode_rewards'][-100:])))
    ...
    def save_model(self, history, model_name, curriculum, agent, with_timestep=False):
        ...
        # Save the tf model
        if with_timestep:
            agent.save_model(name='{}_{}'.format(model_name, len(history['episode_timesteps'])), folder='saved', with_onnx=False)
        else:
    ...
    def get_experience(self, env, num_discriminator_exp=None, verbose=False, random=False):
        ...
        for ep in range(num_discriminator_exp):
            ...
            while True:
                ...
                state_n, step_reward, terminal = env.step(actions=action)
                if self.reward_model is not None:
